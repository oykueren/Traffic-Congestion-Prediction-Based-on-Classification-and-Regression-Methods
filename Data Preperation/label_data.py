"""
This script processes CSV files containing taxi GPS data by applying the Spatio-Temporal DBSCAN clustering algorithm 
to classify traffic density levels. The key steps include:

1. Reading GPS data from CSV files and filtering relevant columns.
2. Converting geographic coordinates (Longitude, Latitude) to UTM coordinates for clustering.
3. Standardizing the data and preparing it for clustering by removing timestamps.
4. Applying Spatio-Temporal DBSCAN clustering with different density parameters (very dense, dense, moderate, low traffic).
5. Assigning labels based on cluster sizes, mapping them to traffic density categories ('Very Dense', 'Dense', 'Moderate', 'Low Traffic').
6. Combining results and setting a consolidated traffic label for each record.
7. Saving the processed data with traffic labels back to CSV files.

The script processes each CSV file in the 'CleanedData' directory and saves the labelled data in the 'Labelled Data' directory.
"""
